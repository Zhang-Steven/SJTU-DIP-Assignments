...
def draw_lines(image, lines, color=[255, 0, 0], thickness=2):
    ...
    for line in lines:
        for x1, y1, x2, y2 in line:
            ...
            if np.absolute(slope) > slope_min:
    ...
    if right_y_set:
        ...
        # 绘制线段
        cv.line(image, (right_x_top, right_y_top),
                (right_x_bottom, max_y), color, thickness)
...
def process_image(image):
    ...
    hof_threshold = 50   # 霍夫平面累加阈值threshold
    min_line_len = 90    # 线段最小长度
    max_line_gap = 50    # 最大允许断裂长度

    kernel_size = 5      # 高斯滤波器大小size
    canny_low_threshold = 2     # canny边缘检测低阈值
    ...
    # cv.imshow("raw2", image)

    # equ = cv.equalizeHist(blur_gray)
    # res = np.hstack((blur_gray, equ))

    # Canny边缘检测
    edge_image = canny(blur_gray, canny_low_threshold, canny_high_threshold)

    # 生成Mask掩模
    vertices = np.array([[(490 * imshape[1] / 1280, imshape[0]), (550 * imshape[1] / 1280, 530 * imshape[0] / 720),
                         (623 * imshape[1] / 1280, 530 * imshape[0] / 720), (710 * imshape[1] / 1280, imshape[0])]], dtype=np.int32)

#    vertices = np.array([[(22 * imshape[1] / 245, imshape[0]), (110 * imshape[1] / 245, 50 * imshape[0] / 272),
#              (158 * imshape[1] / 245, 50 * imshape[0] / 272), (222 * imshape[1] / 245, imshape[0])]], dtype=np.int32)

    masked_edges = region_of_interest(edge_image, vertices)

    cv.imshow("image_raw", masked_edges)
    ...
    cv.imshow("houghline", line_image)
    cv.waitKey()
    '''
    ...
    # drawLines(line_image, lines, thickness=10)
...
if __name__ == '__main__':
    cap = cv.VideoCapture("data/Trim2.mp4")
    label = 1
    while cap.isOpened():
        _, frame = cap.read()
        ...
        name = "/media/st3ven/Seagate Expansion Drive/datasets/processed/Trim2/" + \
            str(label)+".jpg"
        cv.imwrite(name, processed)
        label = label+1
        # cv.waitKey(5)

    # image = cv.imread('picture/test/night.bmp')
